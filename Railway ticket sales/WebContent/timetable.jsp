...
<body>
	...
	<div class="content">
		<ul class="layui-nav">
			...
			<li class="layui-nav-item layui-this"><a href="timetable.jsp">时刻表查询</a></li>
		</ul>
		<div class="down">
			<ul>
				<li>在这里您可以输入您需要查询的火车车次，来查询您乘坐的火车途径的城市以及时刻</li>
				<li>注：普通快车（K/Z）、特快（T）、动车（D）、高铁（G、C）、其他（L/Y等）</li>
			</ul>
			<br>
			<form action="Time_select">
				请输入车次信息：<input type="text" name="time" placeholder="输入格式：K588">
				<input type="submit" value="查询" style="background-color: orange">
			</form>
		</div>
		<%
			LinkedList<Tickets> list = (LinkedList<Tickets>) session.getAttribute("time_table");
